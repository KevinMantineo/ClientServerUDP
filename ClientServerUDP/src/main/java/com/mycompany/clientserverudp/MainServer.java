...
public class MainServer {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println(ANSI_CYAN + "SERVER UDP" + RESET);
            //1) SERVER IN ASCOLTO 
            //si crea il socket e si associa alla porta specifica
            dSocket = new DatagramSocket(port);
            System.out.println(ANSI_CYAN + "Apertura porta in corso!" + RESET);
            ...
            System.out.println(ANSI_YELLOW + "Messaggio ricevuto dal client " + clientAddress +
                    ":" + clientPort + "\n\t" + messageIn + RESET);
            
            //3)RISPOSTA AL CLIENT
            //si prepara il datagramma con i dati da inviare
            messageOut = "Richiesta ricevuta";
            outPacket = new DatagramPacket(messageOut.getBytes(), messageOut.length(), clientAddress, clientPort);
            
            //si inviano i dati
            dSocket.send(outPacket);
            System.out.println(ANSI_CYAN + "Spedito messaggio al client: " + messageOut + RESET);
            ...
            messageOut= "Benvenuti!";
            
            outPacket = new DatagramPacket(messageOut.getBytes(), messageOut.length(), groupAddress, groupPort);
            
            //si inviano i dati
            dSocket.send(outPacket);
            System.out.println(ANSI_CYAN + "Spedito messaggio al gruppo: " + messageOut + RESET);
            }
